<template>
  <div class="sidebar">
    <nav class="sidebar-nav">
      <div slot="header"></div>
      <ul class="nav">
        <li class="nav-item" v-for="(item, index) in navItems" v-if="getUserRestritons(item.restrictionTo)">
          ...
          <template v-else>
            <template v-if="item.children">
              <SidebarNavDropdown :name="item.name" :url="item.url" :icon="item.icon">
                <template v-for="(child, index) in item.children" v-if="getUserRestritons(child.restrictionTo)">
...
export default {
  ...
  methods: {
    ...
    getUserRestritons (restrictionTo) {
      return restrictionTo.indexOf(this.$root.currentUser.role) < 0
    }
  }
}
</script>
